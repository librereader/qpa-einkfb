...
#if 0 // we dont support this properly yet
...
QEinkFbScreen::QEinkFbScreen(const QStringList &args)
    : mArgs(args), mFbFd(-1), mTtyFd(-1), mBlitter(0), mMarker(1)
{
    mMmap.data = 0;
}
...
QRegion QEinkFbScreen::doRedraw()
{
    ...
    for (const QRect &rect : touched) {
        ...
        region.update_region.left = rect.x();
        region.update_region.top = rect.y();
        region.update_region.width = rect.width();
        region.update_region.height = rect.height();
        ...
        region.update_marker = mMarker++;
        region.temp = TEMP_USE_AMBIENT;
        region.flags = 0;

        if (ioctl(mFbFd, MXCFB_SEND_UPDATE, &region) == -1)
            qErrnoWarning("MXCFB_SEND_UPDATE(%d, %d, %d, %d, %d): %s", rect.x(), rect.y(), rect.width(), rect.height(), mMarker);
...
// In einkfb's case it will also include the mouse cursor.
QPixmap QEinkFbScreen::grabWindow(WId wid, int x, int y, int width, int height) const
{
    if (!wid) {
        if (width < 0)
